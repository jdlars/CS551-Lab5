...
function($rootScope, $compile, $animate, $timeout, $ionicTemplateLoader, $ionicPlatform, $ionicBody, IONIC_BACK_PRIORITY) {
   ...
   *  - `{string}` `titleText` The name to show on the action sheet.
...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  return {
     ...
     * @description Gets and sets the current view's name.
     * @param {string=} val The name to update the current view with.
     * @returns {string} Returns the current view's name.
     */
    currentTitle: function(val) {
      if (viewHistory.currentView) {
        if (arguments.length) {
          viewHistory.currentView.name = val;
        }
        return viewHistory.currentView.name;
      }
    },
     ...
     * @description Gets the back view's name.
     * @returns {string} Returns the back view's name.
     */
    backTitle: function(view) {
      var backView = (view && getViewById(view.backViewId)) || viewHistory.backView;
      return backView && backView.name;
    },
...
function($rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory, IONIC_BACK_PRIORITY) {
  ...
  // Set the document name when a new view is shown
  $rootScope.$on('$ionicView.afterEnter', function(ev, data) {
    if (data && data.name) {
      $document[0].name = data.name;
    }
  });
 ...
 * @description If the previous name text should become the back button text. This
 ...
 * @description Which side of the navBar to align the name. Default `center`.
 *
 * @param {string} value side of the navBar to align the name.
 *
 * * `platform`: Dynamically choose the correct name style depending on the platform
 ...
 *
 * * `left`: Left align the name in the navBar
 * * `center`: Center align the name in the navBar
 * * `right`: Right align the name in the navBar.
 *
 * @returns {string} value
 */
 ...
 *   <ion-modal-view>
 *     <ion-header-bar>
 *       <h1 class="name">My Modal name</h1>
 *     </ion-header-bar>
 *     <ion-content>
 *       Hello!
 *     </ion-content>
...
.service('$ionicNavBarDelegate', ionic.DelegateService([
   ...
   * @description Aligns the name with the buttons in a given direction.
   * @param {string=} direction The direction to the align the name text towards.
   ...
   * @description
   * Set the name for the {@link ionic.directive:ionNavBar}.
   * @param {string} name The new name to show.
   */
  'name',
 ...
 *   <ion-popover-view>
 *     <ion-header-bar>
 *       <h1 class="name">My Popover Title</h1>
 *     </ion-header-bar>
 *     <ion-content>
 *       Hello!
 *     </ion-content>
 ...
 *
 *   // .fromTemplate() method
 *   var template = '<ion-popover-view><ion-header-bar> <h1 class="name">My Popover Title</h1> </ion-header-bar> <ion-content> Hello! </ion-content></ion-popover-view>';
 *
  ...
  '<div class="popup-container" ng-class="cssClass">' +
    '<div class="popup">' +
      '<div class="popup-head">' +
        '<h3 class="popup-name" ng-bind-html="name"></h3>' +
        '<h5 class="popup-sub-name" ng-bind-html="subTitle" ng-if="subTitle"></h5>' +
      '</div>' +
...
function($ionicTemplateLoader, $ionicBackdrop, $q, $timeout, $rootScope, $ionicBody, $compile, $ionicPlatform, $ionicModal, IONIC_BACK_PRIORITY) {
  ...
  var $ionicPopup = {
     ...
     *   subTitle: '', // String (optional). The sub-name of the popup.
 ...
 *     </ion-tab>
 *     <ion-tab name="Tab 2">Hello tab 2!</ion-tab>
 *
 *   </ion-tabs>
 * </body>
 * ```
...
function($scope, $element, $attrs, $q, $ionicConfig, $ionicHistory) {
  ...
  var DEFAULT_TITLE = 'default-name';
  var PREVIOUS_TITLE = 'previous-name';
  var HIDE = 'hide';
  ...
  self.name = function(newTitleText) {
    ...
    for (x = 0; x < childNodes.length; x++) {
      ...
      if (c.nodeType == 1) {
        ...
        if (isBackShown && c === backBtnEle) {
          ...
          // not the name, not the back button, not a hidden element
...
function($scope, $element, $attrs, $compile, $timeout, $ionicNavBarDelegate, $ionicConfig, $ionicHistory) {
  ...
  var ITEM_TYPES = 'primaryButtons secondaryButtons leftButtons rightButtons name'.split(' ');
  ...
  self.createHeaderBar = function(isActive) {
    ...
    var headerBarEle = jqLite('<ion-header-bar>').addClass($attrs['class']).attr('align-name', alignTitle);
    if (isDefined($attrs.noTapScroll)) headerBarEle.attr('no-tap-scroll', $attrs.noTapScroll);
    var titleEle = jqLite('<div class="name name-' + alignTitle + '">');
    var navEle = {};
    var lastViewItemEle = {};
    ...
    // append name in the header, this is the rock to where buttons append
    ...
    var headerBarInstance = {
      isActive: isActive,
      name: function(newTitleText) {
        headerBarCtrl.name(newTitleText);
      },
      setItem: function(navBarItemEle, itemType) {
        ...
        if (navBarItemEle) {
          if (itemType === 'name') {
            // clear out the text based name
            headerBarInstance.name("");
          }
  ...
  self.update = function(viewData) {
    ...
    // update the entering header bar's name
    self.name(viewData.name, enteringHeaderBar);
  ...
  self.name = function(newTitleText, headerBar) {
    if (isDefined(newTitleText)) {
      newTitleText = newTitleText || '';
      headerBar = headerBar || getOnScreenHeaderBar();
      headerBar && headerBar.name(newTitleText);
  ...
  self.changeTitle = function(val) {
    deprecatedWarning('changeTitle(val)', 'name(val)');
    self.name(val);
  };
  self.setTitle = function(val) {
    deprecatedWarning('setTitle(val)', 'name(val)');
    self.name(val);
  };
  self.getTitle = function() {
    deprecatedWarning('getTitle()', 'name()');
    return self.name();
  };
  self.back = function() {
...
function($scope, $element, $attrs, $compile, $controller, $ionicNavBarDelegate, $ionicNavViewDelegate, $ionicHistory, $ionicViewSwitcher, $ionicConfig, $ionicScrollDelegate) {
  ...
  self.name = function(val) {
    var associatedNavBarCtrl = getAssociatedNavBarCtrl();
    associatedNavBarCtrl && associatedNavBarCtrl.name(val);
  };
...
function($scope, $element, $attrs, $compile, $rootScope) {
  ...
  self.beforeEnter = function(ev, transData) {
    ...
    if (transData && !transData.viewNotified) {
      ...
      viewTitle = isDefined($attrs.viewTitle) ? $attrs.viewTitle : $attrs.name;
  ...
  function afterEnter() {
    // only listen for name updates after it has entered
    // but also deregister the observe before it leaves
    var viewTitleAttr = isDefined($attrs.viewTitle) && 'viewTitle' || isDefined($attrs.name) && 'name';
    if (viewTitleAttr) {
                ...
                '<div class="action-sheet-wrapper">' +
                  '<div class="action-sheet" ng-class="{\'action-sheet-has-icons\': $actionSheetHasIcon}">' +
                    '<div class="action-sheet-group action-sheet-options">' +
                      '<div class="action-sheet-name" ng-if="titleText" ng-bind-html="titleText"></div>' +
 ...
 *
 * @param {string=} align-name How to align the name. By default the name
 ...
 * @usage
 * ```html
 * <ion-header-bar align-name="left" class="bar-positive">
 *   <div class="buttons">
 ...
 *
 * @param {string=} align-name Where to align the name.
 ...
 *   Some content!
 * </ion-content>
 * <ion-footer-bar align-name="left" class="bar-assertive">
 *   <div class="buttons">
 ...
 *
 * ```html
 *  <ion-footer-bar align-name="left" keyboard-attach class="bar-assertive">
 *    <h1 class="name">Title!</h1>
 *  </ion-footer-bar>
 * ```
 */
    ...
    compile: function(tElement, tAttrs) {
      ...
      for (var x = 0; x < tElement[0].childNodes.length; x++) {
        ...
        if (childNode.nodeType === 1) {
          if (hasIconClass(childNode)) {
            hasIcon = true;
          } else if (childNode.classList.contains('default-name')) {
            hasButtonText = true;
          } else if (childNode.classList.contains('previous-name')) {
      ...
      if (!hasInnerText) {
        ...
        if (!hasButtonText && $ionicConfig.backButton.text()) {
          buttonTextEle.innerHTML += '<span class="default-name">' + $ionicConfig.backButton.text() + '</span>';
        }
        if (!hasPreviousTitle && $ionicConfig.backButton.previousTitleText()) {
          buttonTextEle.innerHTML += '<span class="previous-name"></span>';
        }
 ...
 * @param align-name {string=} Where to align the name of the navbar.
 ...
 * secondary buttons are on the far right, with the header name centered between them.
 * For Android, however, both groups of buttons are on the far right of the header,
 * with the header name aligned left.
 *
 ...
 * @description
 *
 * The nav name directive replaces an {@link ionic.directive:ionNavBar} name text with
 * custom HTML from within an {@link ionic.directive:ionView} template. This gives each
 * view the ability to specify its own custom name element, such as an image or any HTML,
 * rather than being text-only. Alternatively, text-only titles can be updated using the
 * `view-name` {@link ionic.directive:ionView} attribute.
 *
 * Note that `ion-nav-name` must be an immediate descendant of the `ion-view` or
 * `ion-nav-bar` element (basically don't wrap it in another div).
 *
 * @usage
 * ```html
...
.directive('ionNavTitle', ['$document', function($document) {
  return {
    ...
    restrict: 'E',
    compile: function(tElement, tAttrs) {
      var navElementType = 'name';
      var spanEle = $document[0].createElement('span');
      for (var n in tAttrs.$attr) {
        spanEle.setAttribute(tAttrs.$attr[n], tAttrs[n]);
      }
      spanEle.classList.add('nav-bar-name');
      spanEle.innerHTML = tElement.html();
 ...
 *   <!-- The name of the ion-view will be shown on the navbar -->
 *   <ion-view view-name="Home">
 ...
 *
 * ```html
 * <ion-view cache-view="false" view-name="My Title!">
 *   ...
 * </ion-view>
 * ```
 *
  ...
  return {
    ...
    compile: function(element, attr) {
      ...
      return function link($scope, $element, $attr, ctrls) {
        ...
        //Remove name attribute so browser-tooltip does not apear
...
.directive('ionTabNav', [function() {
  return {
    ...
    '<a ng-class="{\'tab-item-active\': isTabActive(), \'has-badge\':badge, \'tab-hidden\':isHidden()}" ' +
      ...
      '<span class="tab-name" ng-bind-html="name"></span>' +
    '</a>',
    scope: {
      name: '@',
      icon: '@',
 ...
 *
 *   <ion-tab name="Home" icon-on="ion-ios-filing" icon-off="ion-ios-filing-outline">
 *     <!-- Tab 1 content -->
 *   </ion-tab>
 *
 *   <ion-tab name="About" icon-on="ion-ios-clock" icon-off="ion-ios-clock-outline">
 *     <!-- Tab 2 content -->
 *   </ion-tab>
 *
 *   <ion-tab name="Settings" icon-on="ion-ios-gear" icon-off="ion-ios-gear-outline">
 *     <!-- Tab 3 content -->
 *   </ion-tab>
 *
 * </ion-tabs>
 ...
 * information, such as its name, whether the back button should be displayed or not, whether the
 ...
 * contain data about the view, such as the name and whether the back button should
 ...
 *
 * @param {string=} view-name A text-only name to display on the parent {@link ionic.directive:ionNavBar}.
 * For an HTML name, such as an image, see {@link ionic.directive:ionNavTitle} instead.
